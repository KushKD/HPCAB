...
import com.dit.himachal.ecabinet.adapter.ArchivedCaseListAdapter;
...
import com.dit.himachal.ecabinet.modal.ArchivedCasePojo;
...
public class SubscribedCases extends AppCompatActivity implements AsyncTaskListenerObjectGet {
    ...
    protected void onCreate (Bundle savedInstanceState){
        ...
        PreventScreenshot.on(SubscribedCases.this);
        ...
        header.setText("Subscribed Cases");
        if (AppStatus.getInstance(SubscribedCases.this).isOnline()) {
            GetDataPojo object = new GetDataPojo();
            object.setUrl(Econstants.url);
            object.setMethord(Econstants.methordGetSubscribedCases);
            object.setMethordHash(Econstants.encodeBase64(Econstants.methordGetSubscribedCasesToken + Econstants.seperator + CommonUtils.getTimeStamp())); //Encode Base64 TODO
            object.setTaskType(TaskType.Get_Subscribed_Cases);
            ...
            object.setBifurcation("Get_Subscribed_Cases"+Preferences.getInstance().user_id);

            new Generic_Async_Get(
                    SubscribedCases.this,
                    SubscribedCases.this,
                    TaskType.Get_Subscribed_Cases).
                    execute(object);
        } else {
            //  CD.showDialog(ArchiveCaseActivity.this,"Internet Not Available");
            DatabaseHandler DB = new DatabaseHandler(SubscribedCases.this);
            Log.e("Get_Subscribed_Cases Start", Integer.toString(DB.GetAllOfflineDataViaFunction(TaskType.Get_Subscribed_Cases.toString(), Preferences.getInstance().user_id, Preferences.getInstance().role_id, "Get_Subscribed_Cases"+Preferences.getInstance().user_id).size()));
            if (DB.GetAllOfflineDataViaFunction(TaskType.Get_Subscribed_Cases.toString(), Preferences.getInstance().user_id, Preferences.getInstance().role_id, "Get_Subscribed_Cases"+Preferences.getInstance().user_id).size() > 0) {
                //Show Events
                try {

                    showData(DB.GetAllOfflineDataViaFunction(TaskType.Get_Subscribed_Cases.toString(), Preferences.getInstance().user_id, Preferences.getInstance().role_id, "Get_Subscribed_Cases"+Preferences.getInstance().user_id).get(0));
                } catch (Exception e) {
                    e.printStackTrace();
                }

            } else {
                CD.showDialogCloseActivity(SubscribedCases.this, Econstants.NO_DATA);
            }
...
//                i.putExtra("EVENTS_DETAILS", cabinet_memo_pojo);
    ...
    private void showData (OfflineDataModel forwarded) throws JSONException {
        ...
        if (forwarded.getFunctionName().equalsIgnoreCase(TaskType.Get_Subscribed_Cases.toString())) {
            ...
            if (json instanceof JSONObject) {
                ...
                if (arrayReports.length() > 0) {
                    ...
                    if (archivedCasePojoList.size() > 0) {
                        archivedCaseListAdapter = new ArchivedCaseListAdapter(SubscribedCases.this, archivedCasePojoList);
                        ...
                        CD.showDialogCloseActivity(SubscribedCases.this, "The Request was Successful. No Records Found.");
                    }
    ...
    public void onTaskCompleted (OfflineDataModel result, TaskType taskType) throws
        ...
        if (taskType == TaskType.Get_Subscribed_Cases) {

            if (result.getHttpFlag().equalsIgnoreCase(Econstants.success)) {
                //Save the rsult to Database
                DatabaseHandler DH = new DatabaseHandler(SubscribedCases.this);
                //Check weather the Hash is Present in the DB or not
                Log.e("??Total Numner of Rows", Integer.toString(DH.getNoOfRowsBeforeOfflineSave(Preferences.getInstance().user_id, Preferences.getInstance().role_id, TaskType.Get_Subscribed_Cases.toString(), "Get_Subscribed_Cases"+Preferences.getInstance().user_id)));
                if (DH.getNoOfRowsBeforeOfflineSave(Preferences.getInstance().user_id, Preferences.getInstance().role_id, TaskType.Get_Subscribed_Cases.toString(), "Get_Subscribed_Cases"+Preferences.getInstance().user_id) == 1) {
                    //Update the Earlier Record
                    DH.updateData(result);
                    Log.e("Updated Row", Boolean.toString(DH.updateData(result)));
                } else if (DH.getNoOfRowsBeforeOfflineSave(Preferences.getInstance().user_id, Preferences.getInstance().role_id, TaskType.Get_Subscribed_Cases.toString(), "Get_Subscribed_Cases"+Preferences.getInstance().user_id) == 0) {
                    DH.addOfflineAccess(result);
                    Log.e("Added Row", Boolean.toString(DH.addOfflineAccess(result)));
                } else {
                    //DELETE ALL THE RECORDS
                    DH.deleteAllExistingOfflineData(Preferences.getInstance().user_id, Preferences.getInstance().role_id, TaskType.Get_Subscribed_Cases.toString(), "Get_Subscribed_Cases"+Preferences.getInstance().user_id);
                    Log.e("Total Records Deleted:-", Integer.toString(DH.deleteAllExistingOfflineData(Preferences.getInstance().user_id, Preferences.getInstance().role_id, TaskType.Get_Subscribed_Cases.toString(), "Get_Subscribed_Cases"+Preferences.getInstance().user_id)));
                ...
                CD.showDialogCloseActivity(SubscribedCases.this, result.getResponse());
            }


        }
        else {
            CD.showDialogCloseActivity(SubscribedCases.this, "Something bad happened. Please connect to the ADMIN team.");
        }
    }
